...
class KalmanFilter:
    def __init__(self):
        ...
        '''
        self.F, self.H = self.init_matrices('acceleration')
    ...
    def init_matrices(self, model_choice, dt=0.1):
...
if __name__ == '__main__':
    ...
    # plot obstacles
    from evaluation.eval_random_simulation import utility
    ...
    i = 0
    pred_start = tau
    while pred_start + pred_len < traj.__len__():
        state_prior = kf.predict(state[i])
        if i < pred_start - 1:
            measurement = traj[i + 1]
            state[i + 1] = kf.correction(state_prior, measurement)
