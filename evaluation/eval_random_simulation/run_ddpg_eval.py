...
def evaluate_ddpg(pos_start=0, pos_end=199, model_name='ddpg', save_dir='../saved_model/',
    ...
    start_goal_pos = pickle.load(open("eval_positions.p", "rb"))    # 1. load 200 start-goal pairs
    ...
    eval = RandEvalGpu(actor_net, robot_init_list, goal_list, poly_list,    # 2. goal_list sent into object RandEvalGpu, let's get into this object
                       max_steps=1000, action_rand=0.01, goal_dis_min_dis=0.3,
                       is_scale=is_scale, is_poisson=is_poisson, use_cuda=use_cuda)
    data = eval.run_ros()
    if is_save_result:
        pickle.dump(data,
                    open('../record_data/' + 'saw_predict_ddpg_' + str(pos_start) + '_' + str(pos_end) + '.p', 'wb+'))
