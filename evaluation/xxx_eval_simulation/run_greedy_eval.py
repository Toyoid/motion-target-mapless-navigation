...
import sys
sys.path.append('../../')
from evaluation.xxx_eval_simulation.greedy_meta_eval import GreedyMetaEval
...
def evaluate_xxx(pos_start=0, pos_end=199, model_name='ddpg', save_dir='../saved_model/',
    ...
    evalution = GreedyMetaEval(controller_net, robot_init_list, target_paths_list, poly_list,
                               max_steps=1000, action_rand=0.01, use_cuda=use_cuda)
    data = evalution.run_ros()
    if is_save_result:
        pickle.dump(data,
                    open('../record_data/' + 'greedy_meta_policy_' + str(pos_start) + '_' + str(pos_end) + '.p', 'wb+'))
