...
class GreedyMetaEval:
    ...
    def __init__(self,
                 ...
                 obstacle_poly_list,
                 ros_rate=10,     # 10
                 max_steps=1000,  # | must change it after making goal moved
                 min_spd=0.05,
                 max_spd=0.5,
                 batch_window=50,
                 action_rand=0.05,
    ...
    def run_ros(self):
        ...
        while not rospy.is_shutdown() and target_ita < run_num:
            ...
            goal_option, encoded_goal = self.greedy_select(pred_target_traj, robot_state[0])
    ...
    @staticmethod
    def greedy_select(traj_predict, robot_pose,
        ...
        goal_option = np.argmin(dist)
        goal_dist = dist[goal_option]
        goal_dir = direction[goal_option]
        # scale distance
        scaled_dist = goal_dist if goal_dist != 0 else goal_dis_min_dis
        scaled_dist = goal_dis_min_dis / scaled_dist
        scaled_dist = goal_dis_scale if scaled_dist > 1 else scaled_dist * goal_dis_scale
        # scale direction
        scaled_dir = goal_dir / goal_dir_range
        encoded_goal = np.array([scaled_dir, scaled_dist])
        return goal_option, encoded_goal
