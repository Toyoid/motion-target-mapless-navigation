...
def load_actor_net(directory, state_num=22, action_num=2, dim=(256, 256, 256)):
    """
    Load actor network for testing
    :param directory: directory of pt file
    :return: actor_net
    """
    actor_net = ActorNet(state_num, action_num,
                         ...
                         hidden2=dim[1],
                         hidden3=dim[2])
    actor_net.load_state_dict(torch.load(directory, map_location=lambda storage, loc: storage))
    return actor_net
...
def gen_robot_pose_n_goal_list(size, poly_list, env_size=((-10, 10), (-10, 10)),
                               goal_th=1.0, robot_goal_diff=5.0):
    """
