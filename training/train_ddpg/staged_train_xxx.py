...
def staged_train_hrl(run_name="HiDDPG", episode_num=(100, 200, 300, 400, 500),
    ...
    while True:
        ...
        while not done and ita_in_episode < ita_per_episode:
            ...
            while step < ctrl_steps and not intrinsic_done and not done and ita_in_episode < ita_per_episode:
                ...
                if overall_steps % save_steps == 0:
                    agent.save("../save_model_weights/save_controller", controller.actor_net, overall_steps // save_steps, run_name + '_ctrl')
            ...
            if env_ita == 4:
                ...
                # Save Model
                if overall_steps % save_steps == 0:
                    agent.save("../save_model_weights/save_meta_controller", meta_controller.actor_net,
        ...
        if overall_episode == 599:
            agent.save("../save_model_weights/save_controller", controller.critic_net, overall_steps // save_steps, run_name + '_critic_of')
        if overall_episode == 999:
            agent.save("../save_model_weights/save_controller", controller.critic_net, overall_steps // save_steps, run_name + '_critic_of')
        if overall_episode == 1499:
            agent.save("../save_model_weights/save_meta_controller", meta_controller.actor_net, 0, run_name + '_meta_ctrl')
            print("Meta Controller Training Finished ...")
            agent.save("../save_model_weights/save_controller", controller.actor_net, 0, run_name + '_ctrl')
