...
4. memory size for meta-controller has been changed into 10000, the previous is too large because of ignoring ctrl_steps or train step(10) 
...
def train_meta_controller(run_name="Train_Meta_Controller", episode_num=500,
    ...
    step_penalty = 0.02    # hyper-parameter, maybe need adjusting
    ...
    while True:
        ...
        while not done and ita_in_episode < ita_per_episode:
            ...
            while step < ctrl_steps and not done and ita_in_episode < ita_per_episode:
                ...
                next_robot_state, reward, done = env.step(action, pred_target_traj[goal_option], pred_target_traj[49],
            ...
            total_ctrl_reward = total_ctrl_reward - step_penalty * ctrl_steps
            episode_reward += total_ctrl_reward
            ...
            if overall_steps % save_steps == 0:
                agent.save("../save_model_weights/save_meta_controller", meta_controller.actor_net, overall_steps // save_steps, run_name)

        print("Episode: {}/{}, Total Reward: {}, Avg Reward: {}, Steps: {}"
              .format(overall_episode, episode_num, episode_reward, episode_reward / (ita_in_episode + 1),
                      ita_in_episode + 1))
        tb_writer.add_scalar('XXX/avg_reward', episode_reward / (ita_in_episode + 1), overall_steps)
        tb_writer.add_scalar('XXX/episode_reward', episode_reward, overall_steps)
        if ita_per_episode < iteration_num_max:
            ita_per_episode += iteration_num_step
        if overall_episode == episode_num - 1:
            agent.save("../save_model_weights/save_meta_controller", meta_controller.actor_net, 0, run_name)
